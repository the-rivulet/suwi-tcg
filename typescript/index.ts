...
import type { DataInfo, DeckInfo } from "./cglobals.js";
...
for(let i of Object.keys(cardIDList)) { serveStaticFile(`assets/cards/${i}.png`); }

let data: DataInfo = {accounts: {}};
let onlineUsers: string[] = [];
let loaded = false;
let activeBattles: Record<string, string> = {};
...
io.on("connection", socket => {
  ...
  socket.on("login attempt", (aUsername: string, aPassword: string) => {
  ...
  socket.on("login done", (username: string) => {
    thisUser = username;
  ...
  socket.on("request user info", (otherUser: string) => {
  ...
  socket.on("request battle", (otherUser: string) => {
    console.log(thisUser + " wants to battle " + otherUser + ".");
    if(!onlineUsers.includes(otherUser)) socket.emit("battle ready", "user isn't online");
    else if(!Object.keys(data.accounts).includes(otherUser)) socket.emit("battle ready", "user doesn't exist");
    else if(activeBattles[otherUser]) socket.emit("battle ready", "user is already battling");
    else if(Object.values(data.accounts[thisUser]?.decks).length == 0) socket.emit("battle ready", "you have no decks");
    else if(Object.values(data.accounts[otherUser]?.decks).length == 0) socket.emit("battle ready", "user has no decks");
    else {
  ...
  socket.on("yeah I want to battle!", (sender: string) => {
    if(activeBattles[sender] != thisUser) return; // guess they canceled or disconnected...
    // ok here we go
